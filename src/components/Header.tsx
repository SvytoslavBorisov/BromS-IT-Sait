  ...
  return (
    <header className="absolute md:static top-0 left-0 w-full bg-white shadow-sm z-10 text-black">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16">
              ...
              return (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className={`
                      relative px-3 py-2 text-base font-light transition-colors
                      hover:text-gray-900
        ...
        {/* Mobile menu button */}
        <button
          className="md:hidden p-2 rounded-lg focus:outline-none focus:ring-0"
          ...
          <ul className="flex flex-col space-y-2 p-4">
            {links.map((link) => (
              <li key={link.href}>
                <Link
                  href={link.href}
                  className="block text-base font-light text-gray-700 py-2 focus:outline-none focus:ring-0"

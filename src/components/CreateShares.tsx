...
export default function CreateShares() {
  ...
  const toggle = (id: string) => {
    setSelected((s) => {
      ...
      clientLog("debug", "participant.toggle", { id, selectedCount: copy.size });
      return copy;
    });
  };
  ...
  const handleCreate = async () => {
    clientLog("info", "shares.create.start", {
      type,
      threshold,
    ...
    try {
      ...
      if (type === "CUSTOM") {
        payload = await createCustomShares(
          secret,
          participants,
          Array.from(selected),
          threshold,
        ...
        clientLog("debug", "shares.create.custom.generated", { shares: payload.shares?.length });
        ...
        clientLog("info", "shares.create.custom.saved", { title });
      } else {
        ...
        clientLog("debug", "shares.create.asymmetric.generated", { pem: !!payload.pem });

        downloadAsFile(payload.pem, title + ".cer", "application/x-x509-ca-cert");
        clientLog("info", "shares.create.asymmetric.downloaded", { fileName: title + ".cer" });
        ...
        clientLog("info", "shares.create.asymmetric.saved", { title });
      }

      alert("Доли созданы и отправлены!");
      clientLog("info", "shares.create.success", { type, title });

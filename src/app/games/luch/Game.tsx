 ...
 * ВНИМАНИЕ: никаких абсолютов/оверлеев — панели идут ВНЕ поля.
 * Обновления:
 * - Единая «сцена» с мягким стеклянным каркасом и засветками.
 * - Деликатная точечная сетка на фоне (SVG), не нагружает рендер.
 * - Аккуратные контейнеры для канваса и нижних панелей.
 */
...
export default function Game() {
  ...
  const placeReflector = (
    ...
    setPlacedReflectorsPct((prev) => [
      ...prev,
      { id: genId("place"), kind: "mirror", A: A_pct, B: B_pct },
    ]);
    setInventory((prev) =>
      prev.map((it) =>
  ...
  const updateFilterPos = (id: string, newPct: { x: number; y: number }) => {
    setCirclesPct((prev) =>
      prev.map((c) => (c.id === id && c.kind === "filter" ? { ...c, C: newPct } : c))
    );
  };
      ...
      >
        <defs>
          <pattern id="dots" x="0" y="0" width="3" height="3" patternUnits="userSpaceOnUse">
            <circle cx="1" cy="1" r=".45" fill="#cfd8ff" />
          </pattern>
        </defs>
      ...
      <div style={{ position: "relative", zIndex: 1, paddingInline: 12 }}>
        <div
          style={{
            ...
            boxShadow:
              "0 16px 40px rgba(0,0,0,0.45), inset 0 0 0 1px rgba(255,255,255,0.03)",
            background:
              "radial-gradient(1200px 500px at 10% 100%, rgba(64,112,255,0.06) 0, transparent 60%), " +
              "radial-gradient(1200px 500px at 90% 0%, rgba(88,255,200,0.05) 0, transparent 60%), " +
              "#0c1120",
        ...
        {/* Подпись-сабтайтл сцены (деликатная) */}
        <div
          style={{
            maxWidth: 1280,
            margin: "8px auto 0",
        ...
        >
          Советы: перетаскивай фильтры и вращай зеркала — ищи минимальный путь луча до цели.
        </div>
      </div>
      ...
      >
        <div
          style={{
            ...
            boxShadow: "0 10px 28px rgba(0,0,0,.35), inset 0 0 0 1px rgba(255,255,255,.03)",
            padding: compact ? 8 : 12,
          }}
        >
          <Inventory

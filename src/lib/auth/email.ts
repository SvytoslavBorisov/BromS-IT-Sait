...
const transporter = createTransport({
  host: process.env.SMTP_HOST!,                      // mail.broms-it.ru
  port: Number(process.env.SMTP_PORT || 587),        // 465 или 587
  secure: Number(process.env.SMTP_PORT) === 465,     // true только для 465
  auth: {
    user: process.env.SMTP_USER!,                    // noreply@broms-it.ru
    pass: process.env.SMTP_PASS!,                    // пароль
  },
  requireTLS: Number(process.env.SMTP_PORT) === 587, // для 587 обязательно TLS
  connectionTimeout: 15000,
  socketTimeout: 15000,
});

// ===== Функция: письмо подтверждения e-mail =====
...
// ===== Функция: письмо для сброса пароля =====

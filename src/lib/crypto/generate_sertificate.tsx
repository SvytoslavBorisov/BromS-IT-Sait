...
export type IssueOptions = {
  ...
  /** Новые поля для включения CDP/AIA */
  crlDpUrl?: string;   // пример: "https://broms-it.ru/crl/ca.crl"
  ocspUrl?: string;    // пример: "https://broms-it.ru/ocsp"
};
...
const OID = {
  ...
  BC:                 "2.5.29.19",
  AKI:                "2.5.29.35",
  CDP:                "2.5.29.31",          // CRL Distribution Points
  AIA:                "1.3.6.1.5.5.7.1.1",  // Authority Information Access
  PKIX_OCSP:          "1.3.6.1.5.5.7.48.1", // id-ad-ocsp
};
...
/** CRL Distribution Points (2.5.29.31) — SEQUENCE OF DistributionPoint(fullName=URI)
 * ВАЖНО: у DistributionPoint есть внешний [0] (EXPLICIT) и внутренний fullName [0] (EXPLICIT) */
function extCRLDistributionPoints(uri: string) {
  const gnURI   = tlv(0x86, IA5String(uri).slice(2));   // GeneralName: uniformResourceIdentifier [6] IA5String
  const fullName = tlv(0xA0, SEQ(gnURI));                // DistributionPointName: fullName [0] EXPLICIT → SEQ of GeneralNames
  const dpField  = tlv(0xA0, fullName);                  // DistributionPoint.distributionPoint [0] EXPLICIT
  const dp       = SEQ(dpField);                         // DistributionPoint ::= SEQUENCE { distributionPoint [0] ... }
  const dpSeq    = SEQ(dp);                              // SEQUENCE OF DistributionPoint
  return SEQ(OID_DER(OID.CDP), OCTET_STRING(dpSeq));     // Extension: SEQ{ oid, extnValue=OCTET STRING(DER) }
}

// --- AIA (1.3.6.1.5.5.7.1.1): AccessDescription = SEQ{ OID(ocsp), GeneralName(URI) }
function extAuthorityInfoAccessOcsp(uri: string) {
  const ad    = SEQ(OID_DER(OID.PKIX_OCSP), tlv(0x86, IA5String(uri).slice(2))); // [6] URI
  const aia   = SEQ(ad);                                    // SEQUENCE OF AccessDescription
  return SEQ(OID_DER(OID.AIA), OCTET_STRING(aia));          // extnValue = OCTET STRING(DER)
}
...
export async function issueGost256Certificate(opts: IssueOptions): Promise<IssueResult>{
  ...
  // CRL Distribution Points (не critical)
  const ext_cdp = crlDpUrl ? extCRLDistributionPoints(crlDpUrl) : undefined;

  // AIA / OCSP (не critical)
  const ext_aia = ocspUrl ? extAuthorityInfoAccessOcsp(ocspUrl) : undefined;

  // Extensions контейнер в TBSCertificate
  const extsList = [ext_ski, ext_ku, ext_bc, ext_aki, ext_cdp, ext_aia].filter(Boolean) as Uint8Array[];
  const extensions = tlv(0xA3, SEQ(...extsList));
...
const res = await issueGost256Certificate({
  issuerCertDerOrPem: issuerPemOrDer,
  issuerPrivateKeyHex: "<hex приватный ключ издателя>",
  subjectPrivateKeyHex: "<hex субъекта>",  // или subjectPublicQxHex/QyHex
  subjectEmail: "user@example.com",
  subjectCN: "Владимир Борисов",
  notBefore: new Date("2025-08-21T00:00:00Z"),
  notAfter:  new Date("2026-08-21T00:00:00Z"),
  curve: CryptoProA_2012_256 as unknown as Gost256CurveParams,
  streebog256,
  gost3410_2012_256_sign: (e,d,p)=>gostSigner256(e,d, CryptoProA_2012_256 as any),

  // вот сюда твой CRL:
  crlDpUrl: "https://broms-it.ru/crl/ca.crl",

  // если поднимешь OCSP — добавь:
  // ocspUrl: "https://broms-it.ru/ocsp",
});
*/
